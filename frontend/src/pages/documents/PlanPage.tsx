...
const objectiveMethodLabels: Record<string, string> = {
  ...
  AUTOMATIC: 'Progreso Automático',
  ACTIVITY_BASED: 'Progreso por Actividades',
};
...
function SstObjectivesAdvancedPanel({ token, readOnly, onComplianceChange, onDirtyChange, saveRequest, discardRequest, onSaved }: { token: string; readOnly?: boolean; onComplianceChange: (status: ResponsableSstComplianceStatus) => void; onDirtyChange: (dirty: boolean) => void; saveRequest: number; discardRequest: number; onSaved: () => void }) {
  ...
  return <div className="advanced-management advanced-management--objectives">
    ...
    <div className="advanced-tabs" role="tablist">{['Panel', 'Objetivos', 'Actividades', 'Alertas', 'Historial'].map((name) => <Button key={name} type="button" variant={tab === name ? 'primary' : 'secondary'} onClick={() => setTab(name)}>{name}</Button>)}</div>

    {tab === 'Panel' ? <section className="advanced-management__section"><div className="objective-card-grid">{record.objectives.map((objective) => {
...
function AnnualWorkPlanPanel({ token, readOnly, onComplianceChange, onDirtyChange, saveRequest, discardRequest, onSaved }: { token: string; readOnly?: boolean; onComplianceChange: (status: ResponsableSstComplianceStatus) => void; onDirtyChange: (dirty: boolean) => void; saveRequest: number; discardRequest: number; onSaved: () => void }) {
  const [record, setRecord] = useState<SstObjectivesAdvancedModel | null>(null);
  const [tab, setTab] = useState('Ejecución Plan Anual');
  const [dirty, setDirty] = useState(false);
  ...
  return <div className="advanced-management advanced-management--objectives">
    ...
    <div className="advanced-tabs" role="tablist">{['Ejecución Plan Anual', 'Actividades y tareas', 'Panel personal', 'Registro Ejecución', 'Alertas', 'Panel Riesgos', 'Historial'].map((name) => <Button key={name} type="button" variant={tab === name ? 'primary' : 'secondary'} onClick={() => setTab(name)}>{name}</Button>)}</div>
    {tab === 'Ejecución Plan Anual' ? <section className="advanced-management__section"><div className="advanced-doc-grid"><article className="advanced-doc-card"><strong>Total Tareas</strong><span>{tasks.length}</span></article><article className="advanced-doc-card"><strong>Completadas</strong><span>{completed}</span></article><article className="advanced-doc-card"><strong>Retrasadas</strong><span>{delayed}</span></article><article className="advanced-doc-card"><strong>Próximas</strong><span>{upcoming}</span></article><article className="advanced-doc-card"><strong>Críticas</strong><span>{critical}</span></article></div>{tasks.map(({ task }) => <article key={task.taskId} className="objective-card"><h3>{task.name}</h3><p className="muted">{task.responsibleUser} · {task.priority} · vence {toDateInputValue(task.dueDate)}</p><div className="objective-progress"><div className="objective-progress__track"><span className={objectiveProgressClass(task.progress)} style={{ width: `${task.progress}%` }} /></div><strong>{task.progress}%</strong></div></article>)}</section> : null}
    {tab === 'Actividades y tareas' ? <section className="advanced-management__section"><div className="actions"><select className="input" value={selectedObjectiveId} onChange={(event) => { setSelectedObjectiveId(event.target.value); setSelectedActivityId(record.objectives.find((item) => item.objectiveId === event.target.value)?.activities[0]?.activityId ?? ''); }}>{record.objectives.map((objective) => <option key={objective.objectiveId} value={objective.objectiveId}>{objective.name}</option>)}</select><select className="input" value={selectedActivityId} onChange={(event) => setSelectedActivityId(event.target.value)}>{(record.objectives.find((item) => item.objectiveId === selectedObjectiveId)?.activities ?? []).map((activity) => <option key={activity.activityId} value={activity.activityId}>{activity.name}</option>)}</select><Button type="button" disabled={readOnly} onClick={addActivity}>Crear actividad</Button><Button type="button" disabled={readOnly} onClick={addTask}>Crear tarea</Button></div><table className="table"><thead><tr><th>Tarea / Descripción</th><th>Responsable</th><th>Vence</th><th>Prioridad</th><th>Costo Est.</th><th>Estado</th><th>Progreso</th><th>Evidencia / Subtareas / Reprogramación</th></tr></thead><tbody>{tasks.map((location) => <tr key={location.task.taskId}><td><input className="input" disabled={readOnly} value={location.task.name} onChange={(event) => setTask(location, { name: event.target.value })} /><textarea className="input" disabled={readOnly} value={location.task.description ?? ''} onChange={(event) => setTask(location, { description: event.target.value })} /></td><td><select className="input" disabled={readOnly} value={location.task.responsibleUser} onChange={(event) => setTask(location, { responsibleUser: event.target.value, assignmentDate: new Date().toISOString().slice(0, 10) })}><option>ADMIN</option><option>MEMBER</option></select><small>Asignación: {toDateInputValue(location.task.assignmentDate)}</small></td><td><input className="input" type="date" disabled={readOnly} value={toDateInputValue(location.task.dueDate)} onChange={(event) => setTask(location, { dueDate: event.target.value })} /></td><td><select className="input" disabled={readOnly} value={location.task.priority} onChange={(event) => setTask(location, { priority: event.target.value as SstObjectiveTaskModel['priority'] })}>{['Low','Medium','High','Critical'].map((p) => {
            const pLabels: Record<string, string> = { Low: 'Baja', Medium: 'Media', High: 'Alta', Critical: 'Crítica' };
            return <option key={p} value={p}>{pLabels[p]}</option>;
          })}</select></td><td><input className="input" type="number" disabled={readOnly} value={location.task.estimatedCost ?? 0} onChange={(event) => setTask(location, { estimatedCost: Number(event.target.value) })} /><input className="input" disabled={readOnly} placeholder="Notas" value={location.task.notes ?? ''} onChange={(event) => setTask(location, { notes: event.target.value })} /></td><td><select className="input" disabled={readOnly} value={location.task.status} onChange={(event) => setTask(location, { status: event.target.value as SstObjectiveTaskModel['status'] })}>{['Pending','In Progress','Completed','Delayed','Cancelled'].map((s) => {
            const sLabels: Record<string, string> = { Pending: 'Pendiente', 'In Progress': 'En Progreso', Completed: 'Completada', Delayed: 'Retrasada', Cancelled: 'Cancelada' };
            return <option key={s} value={s}>{sLabels[s]}</option>;
          })}</select></td><td><select className="input" disabled={readOnly} value={location.task.progress} onChange={(event) => setTask(location, { progress: Number(event.target.value), status: Number(event.target.value) === 100 ? 'Completed' : Number(event.target.value) > 0 ? 'In Progress' : 'Pending' })}>{[0,25,50,75,100].map((progress) => <option key={progress} value={progress}>{progress}%</option>)}</select><div className="objective-progress"><div className="objective-progress__track"><span className={objectiveProgressClass(location.task.progress)} style={{ width: `${location.task.progress}%` }} /></div></div></td><td><div className="actions"><Button type="button" variant="secondary" disabled={readOnly} onClick={() => setTask(location, { subtasks: [...location.task.subtasks, { subtaskId: `sub-${Date.now()}`, name: 'Nueva subtarea', status: 'Pending', progress: 0 }] })}>Subtarea</Button><label className="button button--secondary"><input type="file" style={{ display: 'none' }} disabled={readOnly} accept=".pdf,.xls,.xlsx,image/*,video/*,.doc,.docx" onChange={(event) => { const file = event.target.files?.[0]; if (file) setTask(location, { evidence: [...location.task.evidence, { evidenceId: `ev-${Date.now()}`, fileName: file.name, fileType: file.type || file.name.split('.').pop() || 'document', uploadedAt: new Date().toISOString() }] }); }} />Evidencia</label><Button type="button" variant="secondary" disabled={readOnly} onClick={() => setTask(location, { justifications: [...location.task.justifications, { justificationId: `jus-${Date.now()}`, reason: 'Otro', comments: 'Razón de incumplimiento', date: new Date().toISOString() }] })}>Justificar</Button><Button type="button" variant="secondary" disabled={readOnly} onClick={() => setTask(location, { reschedules: [...location.task.reschedules, { requestId: `res-${Date.now()}`, newDueDate: location.task.dueDate, correctiveAction: 'Acción correctiva', comments: '', status: location.task.priority === 'Critical' ? 'Pendiente Aprobación Gerencial' : 'Approved', reviewedAt: location.task.priority === 'Critical' ? undefined : new Date().toISOString() }] })}>Reprogramar</Button></div>{location.task.reschedules.filter((request) => request.status === 'Pendiente Aprobación Gerencial').map((request) => <div className="actions" key={request.requestId}><small>Aprobación gerencial: {toDateInputValue(request.newDueDate)}</small><Button type="button" variant="secondary" disabled={readOnly} onClick={() => setTask(location, { reschedules: location.task.reschedules.map((item) => item.requestId === request.requestId ? { ...item, status: 'Approved', reviewedAt: new Date().toISOString(), managerComments: 'Approved reschedule' } : item), dueDate: request.newDueDate })}>Aprobar</Button><Button type="button" variant="danger" disabled={readOnly} onClick={() => setTask(location, { reschedules: location.task.reschedules.map((item) => item.requestId === request.requestId ? { ...item, status: 'Rejected', reviewedAt: new Date().toISOString(), managerComments: 'Rejected reschedule' } : item) })}>Rechazar</Button></div>)}<small>Evidencias: {location.task.evidence.length} · Justificaciones: {location.task.justifications.length} · Reprogramaciones: {location.task.reschedules.length}</small></td></tr>)}</tbody></table></section> : null}
    {tab === 'Panel personal' ? <section className="advanced-management__section"><h3>Panel de Tareas Personales</h3>{tasks.map(({ task }) => <article key={task.taskId} className="advanced-list__item"><strong>{task.responsibleUser}: {task.name}</strong><p>{task.status} · {task.progress}% · vence {toDateInputValue(task.dueDate)}</p></article>)}</section> : null}
    {tab === 'Registro Ejecución' ? <section className="advanced-management__section"><div className="grid grid-2"><select className="input" value={selectedObjectiveId} onChange={(event) => setSelectedObjectiveId(event.target.value)}>{record.objectives.map((objective) => <option key={objective.objectiveId} value={objective.objectiveId}>{objective.name}</option>)}</select>{(['progressNotes','achievements','difficulties','observations','nextActions'] as const).map((field) => <textarea key={field} className="input" disabled={readOnly} placeholder={field} value={logDraft[field]} onChange={(event) => setLogDraft({ ...logDraft, [field]: event.target.value })} />)}</div><Button type="button" disabled={readOnly} onClick={addExecutionLog}>Registrar en Bitácora</Button>{record.objectives.flatMap((objective) => (objective.executionLog ?? []).map((log) => ({ objective, log }))).sort((a, b) => new Date(b.log.date ?? '').getTime() - new Date(a.log.date ?? '').getTime()).map(({ objective, log }) => <article className="advanced-list__item" key={log.logId}><strong>{objective.name}</strong><p>{new Date(log.date ?? '').toLocaleString()} · {log.userEmail ?? 'Usuario responsable'}</p><p>{log.progressNotes} {log.achievements} {log.difficulties} {log.observations} {log.nextActions}</p></article>)}</section> : null}
    {tab === 'Alertas' ? <section className="advanced-management__section"><h3>Notificaciones</h3><table className="table"><thead><tr><th>Tipo</th><th>Mensaje</th><th>Destinatarios</th><th>Fecha</th></tr></thead><tbody>{record.alerts.map((alert) => <tr key={`${alert.type}-${alert.objectiveId}-${alert.message}`}><td>{alert.type}</td><td>{alert.message}</td><td>{alert.recipients.join(', ')}</td><td>{new Date(alert.dueAt).toLocaleDateString()}</td></tr>)}</tbody></table></section> : null}
    {tab === 'Panel Riesgos' ? <section className="advanced-management__section"><h3>Tareas que requieren atención</h3>{riskTasks.map(({ task }) => <article key={task.taskId} className="advanced-list__item"><strong>{task.name}</strong><p>Vencida: {new Date(task.dueDate) < new Date() ? 'Sí' : 'No'} · Sin progreso en 30 días: {task.lastProgressAt && (Date.now() - new Date(task.lastProgressAt).getTime()) > 30 * 86400000 ? 'Sí' : 'No'} · Sin evidencia: {!task.evidence.length ? 'Sí' : 'No'} · Sin justificación: {(isTaskDelayed(task) && !task.justifications.length) ? 'Sí' : 'No'}</p></article>)}</section> : null}
  ...
  return (
    <div className="grid">
      ...
      <EvaluationSection
        ...
        onOpenAdvancedManagement={setAdvancedManagementItem}
      />
      <EvaluationSection title="Gestión Integral del SG-SST (15%)" items={integralManagementItems} sectionId="plan-gestion-integral" readOnly={readOnly} onOpenAdvancedManagement={(item) => { if (item.code === '2.6.1') { navigate('/accountability'); } else { setAdvancedManagementItem(item); } }}>
