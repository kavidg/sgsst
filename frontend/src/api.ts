...
export const attachComplianceCredentialDocument = (token: string, payload: AttachComplianceCredentialDocumentPayload) => apiFetch<{ document: ComplianceCredentialDocumentModel; ocr: ComplianceCredentialOcrModel[] }>('/compliance-credentials/documents/attach', token, { method: 'POST', body: JSON.stringify(payload) });
...
export interface ArlAffiliationsAdvancedModel {
  _id: string;
  itemCode: string;
  employees: Array<{ employeeId: string; employeeName: string; document?: string; position?: string; arlName?: string; riskClass?: string; affiliationStatus?: string; affiliationDate?: string }>;
...
export interface SpecialPensionAdvancedModel {
  ...
  itemCode: string;
  enabled: boolean;
  records: Array<{ employeeId: string; employeeName: string; position?: string; highRiskType?: string; requiresSpecialContribution?: boolean; contributionStatus?: string; startDate?: string }>;
...
export interface TrainingManagementAdvancedModel {
  _id: string; itemCode: string; annualProgram: any[]; inductions: any[]; reinductions: any[]; trainings: any[]; attendanceEvidence: string[]; signatureEvidence: string[]; alerts: string[]; history: any[];
}
...
export const approveTrainingManagementAdvanced = (token: string, payload: { status: 'APPROVED'|'REJECTED'|'ADJUSTMENTS_REQUESTED'; comments?: string }) => apiFetch<TrainingManagementAdvancedModel>('/phva-advanced/training-management/approve', token, { method: 'POST', body: JSON.stringify(payload) });

export interface CopasstPeriodModel { _id: string; periodName: string; startDate: string; endDate: string; status: 'ACTIVO'|'PROXIMO_A_VENCER'|'VENCIDO'|'ARCHIVADO'; members: Array<{ userId: string; userName: string; committeeRole: string }>; }
...
export const addCopasstMember = (token: string, periodId: string, payload: { userId: string; userName: string; committeeRole: string; representationType: string; principalType: string; startDate: string }) => apiFetch('/copasst/periods/${periodId}/members', token, { method: 'POST', body: JSON.stringify(payload) });

export interface CommitteePeriodModel extends CopasstPeriodModel { committeeType?: 'COPASST'|'CONVIVENCIA'|'BRIGADA'|'OTHER'; regulations?: Array<Record<string, unknown>>; }
...
export const addCommitteeMember = (token: string, periodId: string, payload: { userId: string; userName: string; committeeRole: string; representationType: string; principalType: string; startDate: string }) => apiFetch(`${'/committee-engine/periods'}/${periodId}/members`, token, { method: 'POST', body: JSON.stringify(payload) });
...
export const assignSstPolicySocializationAdvanced = (token: string, payload: { mode?: 'all' | 'selected' | 'area'; employeeIds?: string[]; area?: string }) => apiFetch<SstPolicyAdvancedModel>('/phva-advanced/sst-policy/socialize', token, { method: 'POST', body: JSON.stringify(payload) });
