...
const QUICK_QUESTIONS = [
  ...
  '¿Qué cambios requieren análisis de impacto?',
  '¿Qué acciones de control faltan?',
  '¿Tenemos cambios pendientes de aprobación?',
  '¿Qué cambios están rechazados?',
  '¿Tenemos cambios implementados sin seguimiento?',
  '¿Qué debería mejorar la empresa?',
  'Analiza el estándar 2.11.1',
];

export function ChangeManagementAI({ token }: Props) {
  ...
  return (
    <div className="cai-card cai-card--assistant">
      <div className="cai-header">
        <span className="cai-header__icon">🤖</span>
        <div>
          <h4 className="cai-header__title">Asistente IA — Gestión del Cambio</h4>
          <p className="cai-header__subtitle">Consulta inteligente sobre el estándar 2.11.1</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="acq-form" style={{ marginTop: '.75rem' }}>
        <div style={{ display: 'flex', gap: '.5rem' }}>
          <input
            className="input"
            placeholder="Escribe tu pregunta sobre gestión del cambio..."
            value={query}
