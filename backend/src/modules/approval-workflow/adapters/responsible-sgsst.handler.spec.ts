 ...
 */
function buildAdapterLike(handler: ResponsibleSgsstHandler): ApprovalAdapter {
  return {
...
function buildHandler(overrides?: {
  ...
  handler: ResponsibleSgsstHandler;
  approveCalls: unknown[][];
  rejectCalls: unknown[][];
} {
  ...
  const handler = new ResponsibleSgsstHandler(
    service,
...
describe('ResponsibleSgsstHandler', () => {
  ...
  it('traduce los estados locales al ApprovalStatus canónico', () => {
    const { handler } = buildHandler();
    ...
    // APPROVED_AND_SIGNED es un estado compuesto de negocio → APPROVED.
    ...
    assert.equal(handler.mapStatus('ARCHIVED'), ApprovalStatus.ARCHIVED);
  ...
  it('aprueba reutilizando approveResponsableSst con la empresa y el usuario resuelto', async () => {
    ...
    const result = await handler.applyDecision(
      buildContext({
        ...
        metadata: {
          ...
          comments: 'Aprobado por representante legal',
        },
      }),
    );
  ...
  it('rechaza reutilizando rejectResponsableSst con el motivo', async () => {
    const { handler, rejectCalls } = buildHandler();

    const result = await handler.applyDecision(
      buildContext({
        decision: ApprovalDecision.REJECTED,
        reason: 'Falta evidencia de licencia',
      }),
    );
    ...
    assert.equal((args[0] as Types.ObjectId).toString(), COMPANY_ID);
    assert.equal(args[2], 'Falta evidencia de licencia');
    assert.equal((result as { approvalStatus: string }).approvalStatus, 'REJECTED');
  });

  it('rechaza ADJUSTMENTS_REQUESTED con BadRequest controlado (no soportado)', async () => {
    const { handler, approveCalls } = buildHandler();
...
describe('Integración ApprovalWorkflowService + ResponsibleSgsstHandler', () => {
  it('crea la solicitud, aprueba y registra los eventos CREATED + APPROVED', async () => {
    ...
    const handler = new ResponsibleSgsstHandler(phvaService, buildUserModel());
  ...
  it('registra el evento REJECTED al decidir el rechazo sobre la solicitud', async () => {
    ...
    const result = await workflow.decideAndApply(
      ...
      ApprovalEntity.PHVA_ADVANCED,
      RECORD_ID,
      { decision: ApprovalDecision.REJECTED, reason: 'Falta evidencia de licencia' },
      buildActor(),
    );

    assert.equal(rejectCalls.length, 1);
    assert.equal(rejectCalls[0][2], 'Falta evidencia de licencia');
...
describe('ResponsibleSgsstHandler Contract', () => {
