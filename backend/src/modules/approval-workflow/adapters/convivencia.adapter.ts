...
/**
 * Adapter de Convivencia para el Approval Workflow Core.
 *
 * Conecta el motor de aprobaciones con el flujo existente del Comité de
 * Convivencia SIN modificar sus endpoints ni su lógica: reutiliza
 * ConvivenciaService.approve (conserva el cambio a APPROVED_AND_SIGNED,
 * auditHistory, approvedBy y las alertas a admins) y ConvivenciaService.reject
 ...
 *
 * Estados locales (schema ConvivenciaPeriod.approvalStatus):
 ...
 */
@Injectable()
export class ConvivenciaAdapter implements ApprovalAdapter {
  readonly module = ApprovalEntity.CONVIVENCIA;

  constructor(
    @Inject(forwardRef(() => ConvivenciaService))
   ...
   * lógica existente de ConvivenciaService (approve/reject) que conserva el
    ...
    switch (ctx.decision) {
      ...
      case ApprovalDecision.ADJUSTMENTS_REQUESTED:
        throw new BadRequestException(
          'ADJUSTMENTS_REQUESTED is not supported by ConvivenciaAdapter',
        );
    }
  }
  ...
  allowedRoles(): string[] {
    // Roles actuales de aprobación/rechazo del Comité de Convivencia
    // (controller): owner y manager.
