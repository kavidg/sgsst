...
/**
 * Stub de ConvivenciaService que captura las llamadas reales al servicio.
 */
function buildConvivenciaService(overrides?: {
  period?: Record<string, unknown>;
  approveResult?: unknown;
  rejectResult?: unknown;
}): {
  service: ConvivenciaService;
...
describe('ConvivenciaAdapter', () => {
  it('implementa el contrato del ApprovalAdapter (module CONVIVENCIA)', () => {
    const { service } = buildConvivenciaService();
    const adapter = new ConvivenciaAdapter(service);

    assert.equal(adapter.module, ApprovalEntity.CONVIVENCIA);
  ...
  it('aprueba el periodo reutilizando ConvivenciaService.approve', async () => {
    const { service, approveCalls } = buildConvivenciaService();
    const adapter = new ConvivenciaAdapter(service);
  ...
  it('rechaza el periodo reutilizando ConvivenciaService.reject', async () => {
    const { service, rejectCalls } = buildConvivenciaService();
    const adapter = new ConvivenciaAdapter(service);

    const result = await adapter.applyDecision(
      buildContext({
        decision: ApprovalDecision.REJECTED,
        reason: 'Falta acta de elección',
      }),
    );
...
describe('Integración ApprovalWorkflowService + ConvivenciaAdapter', () => {
  it('crea la solicitud, aprueba y registra los eventos CREATED + APPROVED', async () => {
    const { service: convivenciaService, approveCalls } = buildConvivenciaService();
    const adapter = new ConvivenciaAdapter(convivenciaService);
  ...
  it('registra el evento REJECTED al decidir el rechazo sobre la solicitud', async () => {
    const { service: convivenciaService, rejectCalls } = buildConvivenciaService();
    const adapter = new ConvivenciaAdapter(convivenciaService);
    const events: unknown[] = [];
    let latestRequest: ApprovalRequestDocument | null = null;
    ...
    const result = await workflow.decideAndApply(
      ...
      ApprovalEntity.CONVIVENCIA,
      PERIOD_ID,
      { decision: ApprovalDecision.REJECTED, reason: 'Falta acta de elección' },
      buildActor(),
    );
...
describe('ConvivenciaAdapter Contract', () => {
  ...
  createAdapterContractSuite(
    () => {
      const { service } = buildConvivenciaService();
      return new ConvivenciaAdapter(service);
    },
    {
