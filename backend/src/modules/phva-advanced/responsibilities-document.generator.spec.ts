...
/** Stub de PhvaAdvancedService: registra llamadas a generateResponsibilitiesDocument. */
function buildGenerator(overrides?: { generateError?: Error }): {
  generator: ResponsibilitiesDocumentGenerator;
  calls: unknown[];
} {
  const calls: unknown[] = [];
  const phvaAdvancedService = {
    generateResponsibilitiesDocument: async (...args: unknown[]) => {
      calls.push(args[0]);
  ...
  return {
    generator: new ResponsibilitiesDocumentGenerator(phvaAdvancedService),
    calls,
  };
}

describe('ResponsibilitiesDocumentGenerator', () => {
  it('implementa el contrato ApprovalDocumentGenerator con la clave real del flujo', () => {
    ...
    assert.equal(generator.entityType, 'PhvaAdvancedResponsibilities');
  });

  it('declara el alias normalizado PHVA_ADVANCED:RESPONSIBILITIES', () => {
    const { generator } = buildGenerator();
  ...
  it('delega en PhvaAdvancedService.generateResponsibilitiesDocument con el contexto traducido', async () => {
  ...
  it('propaga el error del servicio de negocio sin transformarlo', async () => {
    const { generator } = buildGenerator({
      generateError: new Error('Responsibilities not found'),
    });
...
describe('ApprovalDocumentRegistryService con ResponsibilitiesDocumentGenerator', () => {
