...
/**
 * FASE 35B — Tests de seguridad del controller de casos estadísticos.
 *
 * Patrón 3.1.8/3.1.9 (workplace-sanitary-conditions/waste-management
 * controller.spec.ts): controller con dependencias mockeadas + GUARDS REALES
 * (FirebaseAuthGuard + RolesGuard).
 *
 * Matriz de roles FASE 35B:
 *   WRITE  (create/update/close/reopen/deactivate/reactivate) → owner, admin
 *   READ   (findAll/findOne)                                  → owner, admin, manager
 *
 * Tenant: companyId SIEMPRE resuelto server-side (UsersService), nunca del
 * body (SEC-006).
 */
...
describe('OccupationalDiseaseStatisticalCaseController — seguridad (FASE 35B)', () => {
  let controller: OccupationalDiseaseStatisticalCaseController;
  ...
  beforeEach(async () => {
    ...
    const caseService = {
      ...
      close: async (companyId: Types.ObjectId | string) => {
        serviceCalls.push('close');
        receivedCompanyIds.push(String(companyId));
        return { _id: new Types.ObjectId(), companyId };
      },
      reopen: async (companyId: Types.ObjectId | string) => {
        serviceCalls.push('reopen');
        receivedCompanyIds.push(String(companyId));
        return { _id: new Types.ObjectId(), companyId };
      },
      ...
      reactivate: async (companyId: Types.ObjectId | string) => {
        serviceCalls.push('reactivate');
        receivedCompanyIds.push(String(companyId));
        return { _id: new Types.ObjectId(), companyId };
      },
    };
    ...
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        { provide: OccupationalDiseaseStatisticalCaseService, useValue: caseService },
    ...
    controller = module.get(OccupationalDiseaseStatisticalCaseController);
  ...
  it('SEC-002: manager WRITE → rechazado por RolesGuard (create/update/close/deactivate)', async () => {
  ...
  it('SEC-003: owner WRITE (create) → permitido, companyId resuelto server-side', async () => {
    const request = buildRequest('uid-owner');
    const dto = {
      statisticalCaseId: 'EL-2026-001',
      occupationalQualification: 'QUALIFIED',
  ...
  it('SEC-003b: admin WRITE (update/close/reopen/reactivate) → permitido', async () => {
    ...
    await withRoles(['owner', 'admin'], request, () =>
      (
        controller as unknown as {
          close: (req: RequestWithUser, id: string) => Promise<unknown>;
        }
      ).close(request, id),
    );
    await withRoles(['owner', 'admin'], request, () =>
      (
        controller as unknown as {
          reopen: (req: RequestWithUser, id: string) => Promise<unknown>;
        }
      ).reopen(request, id),
    );
    await withRoles(['owner', 'admin'], request, () =>
      (
        controller as unknown as {
          reactivate: (req: RequestWithUser, id: string) => Promise<unknown>;
        }
      ).reactivate(request, id),
    );

    assert.deepEqual(serviceCalls, ['update', 'close', 'reopen', 'reactivate']);
    assert.ok(receivedCompanyIds.every((c) => c === companyIdA.toHexString()));
  });

  it('SEC-004: manager READ (findAll/findOne) → permitido y scoped a su tenant', async () => {
    const request = buildRequest('uid-manager');
    ...
    assert.deepEqual(serviceCalls, ['findAll', 'findOne']);
    assert.ok(receivedCompanyIds.every((c) => c === companyIdA.toHexString()));
  });
  ...
  it('SEC-008: deactivate también recibe el companyId del token (server-side)', async () => {
    const request = buildRequest('uid-owner');
    const id = new Types.ObjectId().toHexString();
    ...
    assert.deepEqual(serviceCalls, ['deactivate']);
    assert.equal(receivedCompanyIds[0], companyIdA.toHexString());
  });

  // ── Tenant isolation explícito (§TENANT ISOLATION) ──
  it('SEC-009: Company A NO puede leer el caso de Company B (findOne entrega solo su tenant)', async () => {
    // El service mock devuelve null cuando el caso no pertenece al tenant:
    // el controller pasa el companyId del token, nunca otro.
    const request = buildRequest('uid-owner');
    const caseIdB = new Types.ObjectId().toHexString();

    const result = await withRoles(['owner', 'admin', 'manager'], request, () =>
      controller.findOne(request, caseIdB),
    );

    assert.equal(receivedCompanyIds[0], companyIdA.toHexString());
    assert.deepEqual(result, { success: false, error: 'Caso no encontrado' });
  });

  it('SEC-010: cross-tenant employeeId en el DTO no cambia el tenant del servicio', async () => {
    const request = buildRequest('uid-owner');
    const dto = {
      statisticalCaseId: 'EL-2026-009',
      occupationalQualification: 'UNDER_REVIEW',
      employeeId: '607f1f77bcf86cd799439099', // empleado de la compañía B
    };
    ...
    // El service siempre recibe companyIdA (el service valida la referencia
    // del employee contra ese tenant y la rechazará).
    assert.equal(receivedCompanyIds[0], companyIdA.toHexString());
  });
});
